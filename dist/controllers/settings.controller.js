...
exports.sendTestEmail = exports.batchUpdateSettings = exports.upsertSetting = exports.getAllPublicSettings = exports.getSetting = void 0;
...
const getSetting = async (req, res) => {
    ...
    try {
        const result = await db_1.default.query('SELECT * FROM settings WHERE key = $1', [key]);
        ...
        if (setting.key === 'smtp_config' && setting.value && setting.value.password) {
            ...
            try {
                setting.value.password = (0, crypto_1.decrypt)(setting.value.password);
            }
            catch (e) {
...
const getAllPublicSettings = async (req, res) => {
    try {
        const result = await db_1.default.query('SELECT * FROM settings');
...
exports.getAllPublicSettings = getAllPublicSettings;
...
const upsertSetting = async (req, res) => {
    ...
    if (key === 'smtp_config' && value && value.password) {
        ...
        if (value.password !== '••••••••••••') {
            value.password = (0, crypto_1.encrypt)(value.password);
        }
        else {
            // Keep existing password if mask is sent
            const current = await db_1.default.query('SELECT value FROM settings WHERE key = $1', [key]);
    ...
    try {
        ...
        if (finalPassword === '••••••••••••') {
            ...
            if (result.rows.length > 0) {
                finalPassword = (0, crypto_1.decrypt)(result.rows[0].value.password);
            }
        }
        // Check for Gmail
