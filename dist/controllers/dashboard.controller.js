...
const getDashboardStats = async (req, res) => {
    try {
        ...
        // 2. Execute Queries
        const [servicesRes, labourLawsRes, blogsRes, enquiriesRes, chartRes, recentEnquiriesRes] = await Promise.all([
            // Total Services (Active Only)
            db_1.default.query("SELECT COUNT(*) FROM services WHERE is_visible = true"),
            // Total Labour Law Updates (Active Only + Last Updated)
            db_1.default.query("SELECT COUNT(*) as count, MAX(updated_at) as last_updated FROM labour_law_updates WHERE is_visible = true"),
            // Total Blogs (Published/Visible Only + Last Updated)
            db_1.default.query("SELECT COUNT(*) as count, MAX(created_at) as last_updated FROM blogs WHERE is_visible = true"),
            // Total Enquiries (All Time)
            db_1.default.query('SELECT COUNT(*) FROM enquiries'),
            // Chart Data (Filtered by Time Range)
            db_1.default.query(`SELECT status, COUNT(*) as count 
                 FROM enquiries 
                 WHERE created_at >= $1 AND created_at <= $2
                 GROUP BY status`, [startDate, endDate]),
            // Recent Enquiries (Latest 4)
            db_1.default.query(`SELECT id, name, service_interest, created_at, status 
                 FROM enquiries 
