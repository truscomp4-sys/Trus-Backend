...
const seedDb = async () => {
    try {
        ...
        for (const s of services) {
            ...
            if (res.rowCount && res.rowCount > 0) {
                ...
                for (let i = 0; i < s.problems.length; i++) {
                    await db_1.default.query('INSERT INTO service_problems (service_id, problem_text, sort_order) VALUES ($1, $2, $3)', [serviceId, s.problems[i], i]);
                }
                for (let i = 0; i < s.features.length; i++) {
                    await db_1.default.query('INSERT INTO service_features (service_id, title, hint, sort_order) VALUES ($1, $2, $3, $4)', [serviceId, s.features[i].title, s.features[i].hint, i]);
                }
                for (let i = 0; i < s.benefits.length; i++) {
                    await db_1.default.query('INSERT INTO service_benefits (service_id, keyword, text, sort_order) VALUES ($1, $2, $3, $4)', [serviceId, s.benefits[i].keyword, s.benefits[i].text, i]);
                }
                for (let i = 0; i < s.whyTrusComp.length; i++) {
                    await db_1.default.query('INSERT INTO service_why_truscomp (service_id, point_text, sort_order) VALUES ($1, $2, $3)', [serviceId, s.whyTrusComp[i], i]);
                }
            }
        }
        ...
        for (const r of resources) {
            ...
            `, [r.title, r.description, r.release_date, r.effective_date, r.state, r.category, r.download_url, r.speaker_name || null, r.speaker_role || null, r.speaker_org || null]);
        }
